...
#if (RAMULATOR == ENABLE)
// reserve 1 MiB or one page of space
inline constexpr auto VMEM_RESERVE_CAPACITY = std::max<uint64_t>(PAGE_SIZE, 1ull << champsim::lg2(1 * MiB));
...
private:
    ...
    uint64_t next_ppage;
    uint64_t last_ppage; // The last (or end) physical page number of the physical address space
