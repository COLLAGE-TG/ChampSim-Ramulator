...
MEMORY_CONTROLLER::MEMORY_CONTROLLER(double freq_scale, int io_freq, double t_rp, double t_rcd, double t_cas, double turnaround, std::vector<channel_type*>&& ul)
...
long MEMORY_CONTROLLER::operate()
{
    ...
    for (auto& channel : channels)
    {
        ...
        auto wq_occu = static_cast<std::size_t>(std::count_if(std::begin(channel.WQ), std::end(channel.WQ), [](const auto& x)
            { return x.has_value(); }));
        auto rq_occu = static_cast<std::size_t>(std::count_if(std::begin(channel.RQ), std::end(channel.RQ), [](const auto& x)
            { return x.has_value(); }));

        // Change modes if the queues are unbalanced
        if ((! channel.write_mode && (wq_occu >= DRAM_WRITE_HIGH_WM || (rq_occu == 0 && wq_occu > 0))) || (channel.write_mode && (wq_occu == 0 || (rq_occu > 0 && wq_occu < DRAM_WRITE_LOW_WM))))
        {
        ...
        auto iter_next_process = std::min_element(std::begin(channel.bank_request), std::end(channel.bank_request),
            ...
            { return ! rhs.valid || (lhs.valid && lhs.event_cycle < rhs.event_cycle); });
...
void DRAM_CHANNEL::check_collision()
{
    for (auto wq_it = std::begin(WQ); wq_it != std::end(WQ); ++wq_it)
    {
        if (wq_it->has_value() && ! wq_it->value().forward_checked)
        {
            ...
            if (auto found = std::find_if(std::begin(WQ), wq_it, checker); found != wq_it)
            { // Forward check
                wq_it->reset();
            }
            else if (found = std::find_if(std::next(wq_it), std::end(WQ), checker); found != std::end(WQ))
            { // Backward check
                wq_it->reset();
            }
            else
...
bool MEMORY_CONTROLLER::add_rq(const request_type& packet, champsim::channel* ul)
{
    ...
    // Find empty slot
    auto rq_it    = std::find_if_not(std::begin(channel.RQ), std::end(channel.RQ), [](const auto& pkt)
...
bool MEMORY_CONTROLLER::add_wq(const request_type& packet)
{
    ...
    auto wq_it    = std::find_if_not(std::begin(channel.WQ), std::end(channel.WQ), [](const auto& pkt)
           { return pkt.has_value(); });
...
bool MEMORY_CONTROLLER::add_rq(const request_type& packet, champsim::channel* ul)
{
    ...
    // Find empty slot
    if (auto rq_it = std::find_if_not(std::begin(channel.RQ), std::end(channel.RQ), [](const auto& pkt)
            { return pkt.has_value(); });
...
bool MEMORY_CONTROLLER::add_wq(const request_type& packet)
{
    ...
    // search for the empty index
    if (auto wq_it = std::find_if_not(std::begin(channel.WQ), std::end(channel.WQ), [](const auto& pkt)
            { return pkt.has_value(); });
